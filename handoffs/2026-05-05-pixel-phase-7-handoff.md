...
建议下一个阶段先做 **Phase 7.1 — 地牢交互打磨**：

1. 增加 UI 事件回放测试，覆盖移动到下一房间并进入节点。
2. 让陷阱变成可预测消耗，并同步 CLI/Pixel 指标规则。
3. 增加分支房间、钥匙门或可选奖励房间。
4. 清理旧 MapScreen 或把它改成调试视图。

### 5.2 已知风险与未决问题

| 风险 / 未决项 | 影响 | 处理建议 |
|---|---|---|
| 旧 MapScreen 还在代码中 | 维护面稍大 | Phase 7.1 确认 DungeonScreen 稳定后删除或改为调试屏。 |
| 目前没有键盘移动的事件级自动测试 | 单元测试覆盖数据模型，smoke 覆盖启动，但没有模拟移动路径 | 增加 pygame event 回放测试。 |
