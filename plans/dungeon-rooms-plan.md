# Pixel Phase 7-12 — 房间地牢化计划

> **状态**：Phase 7 最小闭环已完成（2026-05-05）；Phase 8 交互回放已完成（2026-05-06）；Phase 9 陷阱消耗已完成（2026-05-06）；Phase 10 支线奖励房间已完成（2026-05-06）；Phase 11 旧地图清理已完成（2026-05-06）；Phase 12 钥匙门支线已完成（2026-05-06）
> **上游**：[`plans/pixel-phases.md`](pixel-phases.md) Phase 7-12
...
- 显示钥匙房和锁住的宝库；Phase 12 起，玩家必须先拿钥匙再进入宝库。
...
- ✅ `tests/unit/test_pixel_phase12.py` 覆盖钥匙房、锁住宝库、一次性钥匙和宝库奖励。
...
Phase 12 在固定小地图里加入钥匙门，但不改 route 主线：

- 主路旁有一个钥匙房，进入后按 Enter 可领取一次钥匙。
- 主路旁有一个锁住的宝库，未拿钥匙时不能进入。
- 拿到钥匙后可进入宝库，领取一次 HP/Gold 奖励。
- 钥匙和宝库奖励都按章节隔离，不能反复刷资源。

## 10. 后续扩展

- 增加更多支线形状或章节化数值，但先保持 route 主线不变。
